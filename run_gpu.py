...
def check_hardware(verbose, set_freq, freq_sm, freq_mem, arch, target_vector, target_tensor):
	...
	gpu_name = ''
	vector_precisions = ['int', 'hp', 'hp2', 'sp', 'dp']
	tensor_cores = False
...
def run_roofline(verbose, name, out, set_freq, freq_sm, freq_mem, arch, target_vector, target_tensor, vector_op, threads, blocks):
	...
	for precision in target_vector:
		...
		# MEM L2
		result =  subprocess.run(["./GPU/Bench/Bench", "--test", "MEM","--target", "L2", "--arch", str(arch), "--precision", precision, "--compute", str(compute_capability), "--threads", str(threads), "--blocks", str(blocks), "--device", str(DEVICE)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		if result.returncode != 0:
			print(result.stderr.decode('utf-8').rstrip())
			sys.exit(23)
	...
	for precision in target_tensor:
		outputs = {}
		# TENSOR FLOPS
		result =  subprocess.run(["./GPU/Bench/Bench", "--test", "FLOPS","--target", "tensor", "--arch", str(arch), "--precision", precision, "--compute", str(compute_capability),"--threads", str(threads), "--blocks", str(blocks), "--device", str(DEVICE)], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
		if result.returncode != 0:
			print(result.stderr.decode('utf-8').rstrip())
			sys.exit(16)
...
def main():
	...
	parser.add_argument('--iterations', default=200, nargs='?', type=int, help='Number of measured mixed benchmark iterations (Default: 200)')
	parser.add_argument('--warmup_iterations', default=25, nargs='?', type=int, help='Number of untimed warm-up iterations (Default: 25)')
	parser.add_argument('--l2_fraction', default=0.50, nargs='?', type=float, help='Fraction of detected L2 capacity used by the L2 benchmark (Default: 0.50)')
